...
template <typename T> struct ExtSegTree {
  ...
  void _upd(sz ql, sz qr, T val, sz &p, sz cl, sz cr) {
    p = clone(p);
    ...
    sz m = (cl + cr) >> 1;
    if (ql <= m)
      _upd(ql, qr, val, l[p], cl, m);
    if (m < qr)
      _upd(ql, qr, val, r[p], m + 1, cr);
  }
  void upd(sz ql, sz qr, T val, sz k) { _upd(ql, qr, val, rt[k], 0, ed); }
  T _get(sz ql, sz qr, sz p, sz cl, sz cr, T lazy) {
    if (!p)
      return lazy * (std::min(qr, cr) - std::max(ql, cl) + 1);
    if (ql <= cl && cr <= qr)
      return v[p] + lazy * (cr - cl + 1);
    sz m = (cr + cl) >> 1;
    lazy += lz[p];
    T res = 0;
    if (ql <= m)
      res += _get(ql, qr, l[p], cl, m, lazy);
    if (m < qr)
      res += _get(ql, qr, r[p], m + 1, cr, lazy);
    return res;
  }
