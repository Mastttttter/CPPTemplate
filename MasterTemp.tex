...
\usepackage{listings}
\usepackage{xcolor}
\usepackage[a4paper, margin=2.0cm, bindingoffset=0.5cm]{geometry}
...
\subsection{小结论}
\[
\lfloor \frac{n}{m} \rfloor \% p \equiv \lfloor \frac{n \% (mp)}{m} \rfloor
\]
\[
...
\[
f(n)=\mathrm{C}_{2n}^{n}-\mathrm{C}_{2n}^{n-1}
\]
\[
f(n)=\frac{\mathrm{C}_{2n}^{n}}{n+1}
\]
\[
f(n)=\frac{f(n-1)\cdot (4n-2)}{n+1}
\]
\[
f(n)=\sum_{i=0}^{n-1}f(i)\cdot(n-i-1)
\]
...
\subsubsection{Catalan的小结论}
长度为n的排列且最长下降子序列长度：<2的排列数=第n个加泰罗尼亚（Catalan）数Cn。
...
对于连通图 𝐺 G，以下三个性质是互相等价的： 𝐺 G 是欧拉图； 𝐺 G 中所有顶点的度数都是偶数（对于有向图，每个顶点的入度等于出度）； 𝐺 G 可被分解为若干条不共边回路的并．

求法：dfs经过的边相连构成一个欧拉回路，所有对于每个点度数都是偶数的图，选取任意点开始dfs即可，对于恰好有两个奇数度数的点的图，选取任意奇数点作起点开始dfs即可得到欧拉路径，对于其他情况无欧拉回路/欧拉路径。
\subsection{最短路}
...
\end{lstlisting}
\section{数据结构}
约定：除了树状数组,笛卡尔树，都以0作为起始下标
\subsection{TwoHeap|对顶堆}
\begin{lstlisting}
template <typename T> struct TwoHeap {
  std::multiset<T> a1; // 1 2 3 4
  std::multiset<T> a2; // 4 6 8 9
  int K;               // kth largest  element
  i64 sum1 = 0, sum2 = 0;
  TwoHeap(int k) { K = k; }
  void add(T x) {
    if (!a1.empty() && x <= *a1.rbegin()) {
      a1.insert(x);
      sum1 += x;
    } else {
      a2.insert(x);
  ...
  int size() { return a1.size() + a2.size(); }
  void tidy() {
    while (!a1.empty() && a2.size() < K) {
      auto t = std::prev(a1.end());
      sum2 += *t;
      sum1 -= *t;
      a2.insert(*t);
      a1.erase(t);
...
%https://leetcode.cn/discuss/post/3276177/geng-kuai-de-sortedlistplus-by-424479543-6whe/

假设我们有一个容器，它需要支持以下四种操作：
...
struct sortedlistPlus {
    ...
    vector<vector<_Tp>> blocks; // blocks,bitCnt长度相同，都是1+小块的个数，≈n/load1
    vector<int> bitCnt;         // blocks每个元素中存的是一小块数据，bitCnt是小块.size()的树状数组
    vector<_Tp> blkMax;         // blkMax[i]中存blocks[i].back()
    vector<_Tp> segMax;         // segMax,segSize长度相同,都是小段的个数 ≈ n/(load1*load2), segMax表示小段内元素最大值
    vector<int> segSize;        // segSize表示小段内非空小块个数,segSize[i]∈[1,2load2-1]
    int elementCnt;
    ...
    void clear() {
        blocks.resize(1); // 0th位置空着,从1st小块开始存数，方便树状数组管理
        bitCnt.resize(1);
        blkMax.resize(1);
    ...
    pair<int, Iterator> lower_bound(_Tp x) {              // 找到x的lower_bound在哪一个小块、块内的位置
        if (segSize.empty()) return {0, blocks[0].end()}; // SL为空
        int l = -1, r = segSize.size() - 1, mid;
        // 1.在小段和小段之间二分，一共n/(load1*load2)个小段，复杂度log(n)-log(load1)-log(load2)
        while (r - l > 1) if (segMax[mid = l + r >> 1] >= x) r = mid; else l = mid;
        // 最终的r就是小段编号segi;

        // 2.在小段内非空小块之间二分，小段内约load2个小块，复杂度log(load2)
        l = r << lg2 | 0;          // 即l = segi * load2X + 1, 小段首的小块;r<<lg2=segi
        r = r << lg2 | segSize[r]; // r表示小段内非空小块的右端点
        while (r - l > 1) if (blkMax[mid = l + r >> 1] >= x) r = mid; else l = mid;
        // 最终的r就是小块编号bi;

        /* 3.在小块内部二分，小块内部约load1个小段，复杂度log(load1),
            总复杂度log(n)-log(load1)-log(load2) + log(load2)  + log(load1) = log(n)*/
        return pair{r, (Iterator)ranges::lower_bound(blocks[r], x)};
    }
    pair<int, Iterator> upper_bound(_Tp x) { // 与lower_bound类似
    ...
    void _rangeBitModify(int b1, int b2) { // 一小段树状数组段内重构
        fill(bitCnt.data() + b1, bitCnt.data() + b2 + 1, 0);
        for (int i = b1; i <= b2; ++i) { // 复杂度O(b2-b1) = O(load2),注意不是O(load2·log(load2))
            if (blocks[i].size()) {
                bitCnt[i] += blocks[i].size(); // 利用blocks[b1~b2].size()重构bitCnt[b1~b2]
                blkMax[i] = blocks[i].back();
            }
            if (i + (-i & i) <= b2 and bitCnt[i])
                bitCnt[i + (-i & i)] += bitCnt[i];
        }
        //   ~(b2&load2X-1)表示b2是一个小段的根节点
        //  ~(b2&load2X*2-1)表示b2不止是一个小段的根节点, 而且它还需要吸收前面某些个小段的贡献
        //  比如b2 = 12*load2X, 那么b2还要加上10*load2X的贡献,因为12*load2X == 10*load2X+lowbit(10*load2X);
        //  如果b2 = 11*load2X, 那么b2小段前面没有任何小段对b2有贡献
        //  如果b2 = 12*load2X, lowb=4*load2X/2, 那么b2要加上(12-2)*load2X和(12-1)*load2X对应根节点的贡献
        for (int lowb = (-b2 & b2) / 2; lowb >= load2X; lowb >>= 1)
            bitCnt[b2] += bitCnt[b2 - lowb]; // 平均O(1),有一半的b2是没有前面小段对它产生贡献的，一半的一半有两个小段对b2段有贡献
    }
    void _expand() { // 整个SL重构
        ...
        for (auto &blk : blocksOld)
            c += blk.size() > 0;
        int segn = (c + load2 - 1) / load2; //  segn新的小段数目
        blocks.reserve(segn * load2X + 1);  //
        bitCnt.reserve(segn * load2X + 1);
        blkMax.reserve(segn * load2X + 1);
        int ec = elementCnt;
        clear(); elementCnt = ec; // SL所有数据全部清空, 保留elementCnt, expand不会改变elementCnt
        segMax.assign(segn, minVal);
        segSize.assign(segn, 0);
        for (int i = 0; auto &block : blocksOld) {      // O(n/load1)
            if (block.size()) {                         // 如果是非空小块
                segMax[i >> lg2 - 1] = block.back();    // 更新第i/load2段的最大值
                segSize[i >> lg2 - 1] += 1;             // 更新第i/load2段的小块数目
                blkMax.emplace_back(block.back());      // 把小块的元素最大值存到blkMax数组
                bitCnt.emplace_back(block.size());      // 把小块的元素数目写到树状数组
                blocks.emplace_back(move(block));       // 移动小块到新的blocks数组。 O(1)
                if ((++i & load2 - 1) == 0 and i < c) { // 新的一段前半段填满了，后半段补空段
                    /*  每填load2个小块就在后面填load2个空块,形成一个前半段放满，后半段为空的树状数组小段
                        小块分裂时可以占用空块, 只要重置对应小段的树状数组，不需要重置整个SL
                        只有当空位全部填满时才重构整个SL，均摊复杂度(load1*load2)^-1 * (n/load1) */
                    blocks.resize(blocks.size() + load2); // 往后填充load2个空块
                    bitCnt.resize(blocks.size());         // 往后填充load2个0
                    blkMax.resize(blocks.size());
                }
            }
        }
        // O(n/load1) 处理bitCnt树状数组前缀和
    ...
    void insert(_Tp x) {
        ...
        // if(it != blocks[bi].end() and *it == x) return  ; // 已经存在
        ++elementCnt;
        for (int i = bi; i < bitCnt.size(); i += -i & i) // 在整个树状数组上更新 log(n/load1)
            bitCnt[i] += 1;
        blocks[bi].insert(it, x);
        int segi = (bi - 1) >> lg2; // 即(bi - 1) // load2X
        chmax(blkMax[bi], x);
        chmax(segMax[segi], x);                     // 更新 segMax[segi]
        if (blocks[bi].size() >= load1X) {          // blocks[bi]分裂
            int bj = (segi << lg2) | segSize[segi]; // bi所在的这一小段的最后一个非空小块;
            int bn = segi + 1 << lg2;               // bi所在的这一小段的最后一个小块(与bj区别在bn包括了空块, 表示树状数组小段的根节点)
            if (blocks.size() <= bn) {              // blocks[bn]还不存在，说明是最后一段，直接在blocks[bi+1]处insert
                blocks.insert(blocks.begin() + bi + 1, vector<_Tp>(blocks[bi].begin() + load1, blocks[bi].end()));
                bitCnt.resize(blocks.size()); // 在尾部加个元素，元素值先不管，因为下面这个小段的树状数组内容要重构
                blkMax.resize(blocks.size());
            } else {
                // 这里不能insert了，因为这不是最后一小段，后面可能还有好多小段，insert会把blocks[bn]推到下一个小段的首位;
                // 把blocks[bi+1~bj]移动到blocks[bi+2~bj+1]，(这个闭区间长度是bj-bi); O(load2)
                vector<_Tp>().swap(blocks[bj+1]);//释放blocks[bj+1]的资源，实际生产环境比较重要
                memmove(blocks.data() + bi + 2, blocks.data() + bi + 1, (bj - bi) * sizeof(vector<_Tp>));
                /*    此时blocks[bi+2]和blocks[bi+1]是同一个对象，把blocks[bi+1]内存清零(清的是&blocks[bi+1]处的内存，也就是
                   blocks.data()+bi+1处,不是blocks[bi+1].data()处的内存) , 防止因为blocks[bi+1]的操作释放blocks[bi+2]的资源*/
                memset(blocks.data() + bi + 1, 0, sizeof(vector<_Tp>));
                blocks[bi + 1] = vector<_Tp>(blocks[bi].begin() + load1, blocks[bi].end());
            }
            blocks[bi].resize(load1); // 把后半块数据给blocks[bi+1]后blocks[bi]缩到原来前半块
            if (++segSize[segi] == load2X) {
                _expand();                                                         // 这一小段树状数组填满了,把SL重构
            } else {                                                               // 重构一小段树状数组
                _rangeBitModify(segi << lg2 | 1, min<int>(bn, bitCnt.size() - 1)); // (segi<<lg2|1) == segi*load2X+1;表示这一小段树状数组的左端点
            }
        }
    }
    void erase(_Tp x) {
        if (segMax.empty() or segMax.back() < x) return; // 检查整个SL是否非空，检查SL内是否存在元素 >= x;
        auto [bi, it] = lower_bound(x);                  // segMax.back() >= x , 所以it不会指向end();
        if (*it > x) return;                             // 如果第一个>=x的元素不是x,说明SL内不存在x,直接返回。
        --elementCnt;
        for (int i = bi; i < bitCnt.size(); i += -i & i) // 在整个树状数组上更新 log(n/load1)
            bitCnt[i] -= 1;
        blocks[bi].erase(it);
        int segi = (bi - 1) >> lg2;
        int bj = (segi << lg2) | segSize[segi]; // bi所在的这一小段的最后一个非空小块;
        if (blocks[bi].empty()) {               // blocks[bi]空了,删除
            int bn = segi + 1 << lg2;           // bi所在的这一小段的最后一个小块(与bj区别在bn包括了空块, 表示树状数组小段的根节点)
            if (blocks.size() <= bn) {          // blocks[bn]还不存在，说明是最后一小段，直接erase(blocks[bi])
                blocks.erase(blocks.begin() + bi);
                blkMax.erase(blkMax.begin() + bi);
                bitCnt.resize(blocks.size());                            // bitCnt.size() 与 blocks.size()保持一致， 其实就是bitCnt.pop_back();
                if (--segSize[segi] == 0) {                              // 删除当前小块后，最后一小段也空了
                    segMax.pop_back(), segSize.pop_back();               // 小段数量减一
                } else {                                                 // 当前小段非空
                    _rangeBitModify(segi << lg2 | 1, bitCnt.size() - 1); // 仅把当前小段的树状数组重构
                    if (bi == bj) segMax[segi] = blocks[bj - 1].back();  // 万一删了最后一小块，会导致segMax[segi]变小，更新一下；
                }
            } else { // 不是最后一小段的情况
                // 把blocks[bi+1~bj]移动到blocks[bi~bj-1]，(这个闭区间长度是bj-bi); O(load2)
                if (--segSize[segi] == 0) { // 删除当前小块后，当前小段也空了
                    _expand();              // 把整个SL重构
                } else {                    // 当前小段非空，仅把当前小段的树状数组重构
                    vector<_Tp>().swap(blocks[bi]);//释放blocks[i]的资源，实际生产环境比较重要
                    memmove(blocks.data() + bi, blocks.data() + bi + 1, (bj - bi) * sizeof(vector<_Tp>));
                    // 此时blocks[bj]和blocks[bj-1]是同一个对象，把blocks[bj]内存清零,防止因为blocks[bj]的操作释放blocks[bj-1]的资源*/
                    memset(blocks.data() + bj, 0, sizeof(vector<_Tp>));
                    _rangeBitModify(segi << lg2 | 1, bn);               // 仅把当前小段的树状数组重构
                    if (bi == bj) segMax[segi] = blocks[bj - 1].back(); // 万一删了最后一小块，会导致segMax[segi]变小，更新一下；
                }
            }
        } else { // blocks[bi]非空, 不需要删除blocks[bi]小块, 小段内结构不变
            blkMax[bi] = blocks[bi].back();
            if (bi == bj) segMax[segi] = blocks[bi].back(); // 万一删了最后一小块的最后一个元素，会导致segMax[segi]变小，更新一下；
        }
    }
    int size() const { return elementCnt; }
    _Tp operator[](int k) { // SL中第k个大的元素，base:0
        assert(k >= 0 and k < elementCnt);
        int bi = 0;
        for (int i = 1 << __lg(bitCnt.size() - 1); i; i >>= 1)
            if ((bi | i) < bitCnt.size() and k - bitCnt[bi | i] >= 0) k -= bitCnt[bi |= i];
        // 如果k减去bitCnt[bi|i]的值后还是一个>=0的下标，那么bi|=i, k减去新的bitCnt[bi]
        //  更新完之后，sum(blocks[1~bi].size())已经被k减去了，k仍>=0; 且此时k-blocks[bi+1].size()将小于0;
        return blocks[bi + 1][k];
    }
    int rank(_Tp x) { //找到lower_bound(x)在整个数组中的下标
        if (segSize.empty()) return 0;
        auto [bi, it] = lower_bound(x);
        int rk = it - blocks[bi].begin(); // 在bi块内的rank + sum(blocks[1~bi-1].size());
    ...
    int count(_Tp x) {
        ...
        for (int i = bi - 1; i; i ^= -i & i)
            rk += bitCnt[i];
        return rk - rank(x); // upper_bound - lower_bound
    }
    pair<int, Iterator> prev(pair<int, Iterator> iter) {
        auto &[bi, it] = iter;
        assert(bi > 1 or bi == 1 and it > blocks[bi].begin());                        // 检测bi>=1, 检测iter不指向SL的首元素
        if (it > blocks[bi].begin()) return {bi, std::prev(it)};                      // prev位于同一小块
        if ((bi & load2X - 1) != 1) return {bi - 1, std::prev(blocks[bi - 1].end())}; // prev位于同一小段的前一个小块

        // prev位于前一小段的最后一个非空小块 , 此时 (bi & load2-1) = 1
        int Segi = ((bi - 1) >> lg2);
        bi = (Segi - 1 << lg2) | segSize[Segi - 1]; // 前一小段的最后一个非空小块
        return {bi, std::prev(blocks[bi].end())};
    }
    _Tp lessEqual(_Tp x){ // 寻找最大的<=x的元素，即*prev(upper_bound)
        auto [bi,it] = upper_bound(x);
        if( not (bi > 1 or bi == 1 and it > blocks[bi].begin()) ) return _Tp(-1);
        return *prev(pair<int, Iterator>{bi,it}).second;
    }
    _Tp greaterEqual(_Tp x){ //寻找最小的>=x的元素，即*lower_bound
        auto [bi,it] = lower_bound(x);
        if(it == blocks[bi].end()) return _Tp(-1);
        return *it;
    }
}; sortedlistPlus<int> sl;
\end{lstlisting}
\subsection{珂朵莉树|ODT}
std::map 实现，其键维护左端点，其值维护其对应的左端点到下一个左端点之前的值。
...
\begin{lstlisting}
template <typename T> struct ExtSegTree {
  using sz = int;
  std::vector<T> v = {0};
  std::vector<T> lz = {0};
  std::vector<sz> l = {0};
  ...
  sz _upd(sz ql, sz qr, T val, sz p, sz cl, sz cr) {
    p = clone(p);
    v[p] += (std::min(qr, cr) - std::max(ql, cl) + 1) * val;
    if (ql <= cl && cr <= qr) {
      lz[p] += val;
      return p;
    }
    sz m = (cl + cr) >> 1;
    if (ql <= m)
      l[p]=_upd(ql, qr, val, l[p], cl, m);
    if (m < qr)
      r[p]=_upd(ql, qr, val, r[p], m + 1, cr);
    return p;
  }
  sz upd(sz ql, sz qr, T val, sz k=0) {
    sz ret=rt.size();
    rt.emplace_back(_upd(ql, qr, val, rt[k], 0, ed));
    return ret;
  }
  T _get(sz ql, sz qr, sz p, sz cl, sz cr, T la) {
    if (!p)
      return la * (std::min(qr, cr) - std::max(ql, cl) + 1);
    if (ql <= cl && cr <= qr)
      return v[p] + la * (cr - cl + 1);
    sz m = (cr + cl) >> 1;
    la += lz[p];
    T res = 0;
    if (ql <= m)
      res += _get(ql, qr, l[p], cl, m, la);
    if (m < qr)
      res += _get(ql, qr, r[p], m + 1, cr, la);
    return res;
  }
  T get(sz ql, sz qr, sz k) { return _get(ql, qr, rt[k], 0, ed, 0); }
  sz build(const std::vector<T> &init, sz cl, sz cr) {
    sz p = clone();
    if (cl == cr) {
      v[p] = init[cl];
      return p;
    ...
    sz m = (cr + cl) >> 1;
    l[p] = build(init, cl, m);
    r[p] = build(init, m + 1, cr);
    v[p] = v[l[p]] + v[r[p]];
    return p;
  }
  ExtSegTree(sz n):ed(n-1) {}
  ExtSegTree(const std::vector<T> &init) : ed((sz)init.size() - 1) {
    rt.emplace_back(build(init, 0, ed));
  }
};
...
template <typename T, typename Merge = MaxOp<T>>
class SegTree2d {
  private:
  int n, m;
  ...
  void build_y(std::vector<std::vector<T>> const &a, int vx, int lx, int rx,
    ...
    int my = (ly + ry) >> 1;
    build_y(a, vx, lx, rx, vy << 1, ly, my);
    build_y(a, vx, lx, rx, vy << 1 | 1, my + 1, ry);
    tree[vx][vy] = merge(tree[vx][vy << 1], tree[vx][vy << 1 | 1]);
  }

  void build_x(std::vector<std::vector<T>> const &a, int vx, int lx, int rx) {
    if (lx != rx) {
      int mx = (lx + rx) >> 1;
      build_x(a, vx << 1, lx, mx);
      build_x(a, vx << 1 | 1, mx + 1, rx);
    }
    build_y(a, vx, lx, rx, 1, 0, m - 1);
  }

  T query_y(int vx, int vy, int ly, int ry, int qly, int qry) const {
    if (qry < ly || ry < qly) {
      return identity;
    }
    if (qly <= ly && ry <= qry) {
    ...
    int my = (ly + ry) >> 1;
    T left = query_y(vx, vy << 1, ly, my, qly, qry);
    T right = query_y(vx, vy << 1 | 1, my + 1, ry, qly, qry);
    return merge(left, right);
  }

  T query_x(int vx, int lx, int rx, int qlx, int qrx, int qly, int qry) const {
    if (qrx < lx || rx < qlx) {
      return identity;
    }
    if (qlx <= lx && rx <= qrx) {
    ...
    int mx = (lx + rx) >> 1;
    T left = query_x(vx << 1, lx, mx, qlx, qrx, qly, qry);
    T right = query_x(vx << 1 | 1, mx + 1, rx, qlx, qrx, qly, qry);
    return merge(left, right);
  }
  ...
  void update_y(int vx, int lx, int rx, int vy, int ly, int ry, int x, int y,
    ...
    int my = (ly + ry) >> 1;
    if (y <= my) {
      update_y(vx, lx, rx, vy << 1, ly, my, x, y, value);
    } else {
      update_y(vx, lx, rx, vy << 1 | 1, my + 1, ry, x, y, value);
    }
    tree[vx][vy] = merge(tree[vx][vy << 1], tree[vx][vy << 1 | 1]);
  }

  void update_x(int vx, int lx, int rx, int x, int y, T const &value) {
    if (lx != rx) {
      int mx = (lx + rx) >> 1;
      if (x <= mx) {
        update_x(vx << 1, lx, mx, x, y, value);
      } else {
        update_x(vx << 1 | 1, mx + 1, rx, x, y, value);
      }
    }
    update_y(vx, lx, rx, 1, 0, m - 1, x, y, value);
  }

  public:
  SegTree2d() = default;

  SegTree2d(std::vector<std::vector<T>> const &a, T identity_,
            Merge merge_ = Merge())
      : identity(identity_),
        merge(merge_) {
    n = a.size();
    m = a[0].size();
    tree.assign(4 * n + 5, std::vector<T>(4 * m + 5, identity));
    build_x(a, 1, 0, n - 1);
  }
  ...
  void update(int x, int y, T const &value) {
    assert(0 <= x && x < n);
    assert(0 <= y && y < m);
    update_x(1, 0, n - 1, x, y, value);
  }
};
  ...
  int countLocalMaximums(std::vector<std::vector<int>> &matrix) {
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < m; ++j) {
        if (matrix[i][j]) {
          auto x = matrix[i][j];
          if (x == std::max(st.query(std::max(0, i - x), std::max(0, j - x + 1),
                                     std::min(n - 1, i + x),
                                     std::min(m - 1, j + x - 1)),
                            st.query(std::max(0, i - x + 1), std::max(0, j - x),
                                     std::min(n - 1, i + x - 1),
                                     std::min(m - 1, j + x)))) {
            ans++;
          }
...
#define READ_N(TYPE, COUNT, CONTAINER)                                         \
  do {                                                                         \
    std::copy_n(std::istream_iterator<TYPE>(cin), (COUNT),                     \
...
signed main() {
  ...
  cin.tie(nullptr);
  i32 o = 1;
  if constexpr (!COMMUNICATION && MULTITEST) {
    cin >> o;
  }
  while (o--) {
...
struct SAM {
  ...
  int extend(int p, int c, char offset = 0) {
    t[0].next[c] = 1;
    return extend_(p, c - offset);
  }
...
class Solution {
  ...
  int smallestUniqueSubarray(std::vector<int> &nums) {
    ...
    for (int v = 2; v < sam.size(); ++v) {
      if (sam.occ(v) == 1) {
        ans = std::min(ans, sam.len(sam.link(v)) + 1);
      }
    }
    return ans;
  }
...
\begin{lstlisting}
struct StringHasher {
  const static i64 base = 131;
  const static i64 mod1 = 212370440130137957, mod2 = 1000000007;
  std::vector<i64> h1, p1, h2, p2;
  StringHasher(const std::string_view s) {
    int n = s.size();
    h1.resize(n + 1, 0);
    h2.resize(n + 1, 0);
    p1.resize(n + 1, 1);
    p2.resize(n + 1, 1);
    for (int i = 0; i < n; ++i) {
      h1[i + 1] = (i64)(((i128)h1[i] * base + s[i]) % mod1);
      h2[i + 1] = (h2[i] * base + (i64)s[i]) % mod2;
      p1[i + 1] = (i64)(((i128)p1[i] * base) % mod1);
      p2[i + 1] = (p2[i] * base) % mod2;
    }
  }
  std::pair<i64, i64> getHash(int l, int r) {
    return {(i64)(((i128)h1[r] - (i128)h1[l] * p1[r - l]) % mod1 + mod1) % mod1,
            (i64)(((i128)h2[r] - (i128)h2[l] * p2[r - l]) % mod2 + mod2) %
                mod2};
  }
  bool operator==(const StringHasher &o) const {
    return h1.back() == o.h1.back() && h2.back() == o.h2.back();
  }
  bool comp(int l1, int r1, int l2, int r2) {
    return getHash(l1, r1) == getHash(l2, r2);
  }
  int lcp(int l1, int r1, int l2, int r2) {
    int f = 0, l = std::min(r1 - l1, r2 - l2);
    int c = l - f;
    while (c) {
      int h = c >> 1;
      int it = f + h;
      if (comp(l1, l1 + it + 1, l2, l2 + it + 1)) {
        f = ++it;
        c -= h + 1;
      } else
        c = h;
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        i64 max = w * s;
        for (int r = 0; r < w; ++r) {
            std::deque<std::pair<int, i64>> deq;
            deq.emplace_back(r, f[r]);
            for (int i = w + r; i <= m; i += w) {
                while (!deq.empty() && i - deq.front().first > max) {
                    deq.pop_front();
                }
                while (!deq.empty() &&
                        deq.back().second + (i - deq.back().first) / w * 1ll * v <=
                        f[i]) {
                    deq.pop_back();
                }
                deq.emplace_back(i, f[i]);
                f[i] = std::max(f[i], deq.front().second +
                                (i - deq.front().first) / w * 1ll * v);
            }
        }
    }
    std::cout << f[m] << '\n';
...
sub_and[0]=0;
\end{lstlisting}
\subsection{SOS(sub of section) DP|高维前缀和|子集和DP}
\begin{lstlisting}
for(i32 i=0;i<wid;++i){
    for(i32 s=0;s<u;++s){
        if (s>>i&1){
...
\subsection{手写高维全缀和}
\begin{lstlisting}
std::array<i64, 1'000'000> pre{};
void solve() {
  int n;
  cin >> n;
  auto getid = [](std::string const &s) {
    int id = 0;
    for (auto &e: s) {
      id *= 10;
      id += e - '0';
  ...
  std::vector<int> step{100000, 10000, 1000, 100, 10, 1};
  for (int di = 0; di < 6; ++di) {
    int st = step[di];
    for (int idx = 0; idx < 1'000'000; ++idx) {
      int di = (idx / st) % 10;
      if (di > 0) {
        pre[idx] += pre[idx - st];
      }
  ...
  auto getv = [&](std::array<int, 6> const &d) {
    for (auto &e: d) {
      if (e < 0) {
        return 0ll;
      }
